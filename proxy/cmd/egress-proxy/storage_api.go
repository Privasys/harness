...
// layer — the sealed Ed25519 key, the AppGrant token, the attested client
// identity Drive matches against the grant's subject. Node asks for a file by
// name and receives bytes; it cannot widen a scope, reach outside the granted
// folder, or mint a token, because it never sees the key.
//
...
func registerStorageAPI(mux *http.ServeMux, capStore *capability.Store, id *capability.Identity,
	...
	storeFor := func() (*capability.DriveStore, string) {
		...
		g := capStore.Granted(sub)
		if !g.Usable() {
			return nil, "this user has not approved a storage folder; sessions are kept in memory only"
		}
		if driveHost == "" {
			return nil, "no Drive host is configured for this deployment"
		}
		ds, err := capability.NewDriveStore(client, driveHost, id, g)
		if err != nil {
